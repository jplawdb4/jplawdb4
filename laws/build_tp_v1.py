...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "app_tp", "data2")
...
t_hou, n_hou, nd_hou = extract_nodes(XMLDIR+'/租税特別措置法.xml', [("66_4","66_4_5")])
t_rei, n_rei, nd_rei = extract_nodes(XMLDIR+'/租税特別措置法施行令.xml', [("39_12","39_12_4")])
t_kis, n_kis, nd_kis = extract_nodes(XMLDIR+'/租税特別措置法施行規則.xml', [("22_10","22_10_7")])
nd_tsu, grp_tsu = extract_tsutatsu_xml(XMLDIR+'/租税特別措置法関係通達_法人税編.xml', ['66の4'])
...
GROUPS = {
 'tp_hou': [("措法 第66条の4関係（移転価格税制）","66_4","66_4_5")],
 'tp_rei': [("措令 第39条の12関係","39_12","39_12_4")],
 'tp_kis': [("措規 第22条の10関係","22_10","22_10_7")],
}
...
def build_tsu(nodes, groups):
    ...
    for n in nodes:
        ...
        if n['type'] == 'Article':
            arts.append(dict(id=n['art'], address=n['address'], label=n['ka'], c=n.get('caption','')))
    return dict(slug='tp_tsu', key='tsu', title='租税特別措置法関係通達（法人税編・移転価格関係）',
                num='国税庁HTML版（snapshot 2025-06-28）', lawzilla='',
                arts=arts, groups=groups, order=order, nodes=nmap), res

hou, res_hou = build_xml_law('hou', 'tp_hou', t_hou+'（移転価格税制抜粋）', n_hou, nd_hou, ('rei','kis'))
rei, res_rei = build_xml_law('rei', 'tp_rei', t_rei+'（移転価格税制抜粋）', n_rei, nd_rei, ('kis',))
kis, res_kis = build_xml_law('kis', 'tp_kis', t_kis+'（移転価格税制抜粋）', n_kis, nd_kis, ())
tsu, res_tsu = build_tsu(nd_tsu, grp_tsu)

r_hou, r_rei, r_kis, r_tsu = res_hou.refs, res_rei.refs, res_kis.refs, res_tsu.refs
for a in tsu['arts']:
    parts = a['id'].split('_'); tgt = None
    for L in range(len(parts), 0, -1):
        cand = 'ln' + '_'.join(parts[:L])
        if cand in known['hou']: tgt = cand; break
    if tgt: r_tsu.append((a['address'], 'hou', tgt, tgt))
...
EXT_XML = {
    ...
    '340AC0000000034': (XMLDIR+'/法人税法.xml', '法人税法'),
    '340AC0000000033': (XMLDIR+'/所得税法.xml', '所得税法'),
    '337AC0000000066': (XMLDIR+'/国税通則法.xml', '国税通則法'),
    '340CO0000000097': (XMLDIR+'/法人税法施行令.xml', '法人税法施行令'),
    '340M50000040012': (XMLDIR+'/法人税法施行規則.xml', '法人税法施行規則'),
