...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "app_ftc", "data2")
...
t_hou, n_hou, nd_hou = extract_nodes(XMLDIR+'/法人税法.xml', [("69","69_2")])
t_rei, n_rei, nd_rei = extract_nodes(XMLDIR+'/法人税法施行令.xml', [("142","142_2")])
t_kis, n_kis, nd_kis = extract_nodes(XMLDIR+'/法人税法施行規則.xml', [("29","29_4")])
nd_tsu, grp_tsu = extract_tsutatsu_xml(XMLDIR+'/法人税基本通達.xml', ['16-3'])
...
GROUPS = {
 'ftc_hou': [("法 第69条関係（外国税額控除）","69","69_2")],
 'ftc_rei': [("令 第142条関係","142","142_2")],
 'ftc_kis': [("規 第29条関係","29","29_4")],
}
...
def build_tsu(nodes, groups):
    ...
    for n in nodes:
        ...
        if n['type'] == 'Article':
            arts.append(dict(id=n['art'], address=n['address'], label=n['ka'], c=n.get('caption','')))
    return dict(slug='ftc_tsu', key='tsu', title='法人税基本通達（外国税額控除関係・16-3）',
                num='国税庁HTML版（snapshot 2025-06-28）', lawzilla='',
                arts=arts, groups=groups, order=order, nodes=nmap), res

hou, res_hou = build_xml_law('hou', 'ftc_hou', t_hou+'（外国税額控除関係抜粋）', n_hou, nd_hou, ('rei','kis'))
rei, res_rei = build_xml_law('rei', 'ftc_rei', t_rei+'（外国税額控除関係抜粋）', n_rei, nd_rei, ('kis',))
kis, res_kis = build_xml_law('kis', 'ftc_kis', t_kis+'（外国税額控除関係抜粋）', n_kis, nd_kis, ())
