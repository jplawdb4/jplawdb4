...
def extract_tsutatsu(files):
    ...
    for path, glabel in files:
        ...
        cur = None; cur_title = ''; pseq = 0; garts = []
        num = ''
        for elem in body.find_all(['h2', 'p', 'ol', 'ul']):
            if elem.name == 'h2':
                cur_title = elem.get_text(strip=True); continue
            ...
            if m:
                ...
                cur = addrpart; pseq = 1; garts.append(cur)
                nodes.append(dict(type='Article', address='ln'+cur, ka=num, title=num,
                                  sentence='', art=cur, caption=cur_title.strip('（）'), tables=[], target=base))
                nodes.append(dict(type='Paragraph', address=f'ln{cur}.{pseq}', ka=num,
                                  title='', sentence=text[m.end():].strip(), art=cur, tables=[]))
            elif cur:
                pseq += 1
...
t_hou, n_hou, nd_hou = extract_nodes('/tmp/sotokuso.xml', [("66_6","66_9_5"),("40_4","40_6")])
t_rei, n_rei, nd_rei = extract_nodes('/tmp/sotochirei.xml', [("39_14","39_20_9"),("25_19","25_27")])
t_kis, n_kis, nd_kis = extract_nodes('/tmp/sotokisoku.xml', [("18_20","18_20_2"),("22_11","22_11_3")])
...
def build_tsu(nodes, groups):
    ...
    return dict(slug='cfc_tsu', key='tsu', title='租税特別措置法関係通達（法人税編・CFC関係）',
                num='昭和50年2月14日直法2-2（国税庁HTML当時版）', lawzilla='',
                arts=arts, groups=groups, order=order, nodes=nmap), res

hou, res_hou = build_xml_law('hou', 'cfc_hou', t_hou+'（CFC税制抜粋）', n_hou, nd_hou, ('rei','kis'))
rei, res_rei = build_xml_law('rei', 'cfc_rei', t_rei+'（CFC税制抜粋）', n_rei, nd_rei, ('kis',))
kis, res_kis = build_xml_law('kis', 'cfc_kis', t_kis+'（CFC税制抜粋）', n_kis, nd_kis, ())
...
EXT_XML = {
    '332AC0000000026': ('/tmp/sotokuso.xml', '措法'),
    '332CO0000000043': ('/tmp/sotochirei.xml', '措令'),
    '332M50000040015': ('/tmp/sotokisoku.xml', '措規'),
    '340AC0000000034': ('/tmp/houjinzei.xml', '法人税法'),
    '340AC0000000033': ('/tmp/shotokuzei.xml', '所得税法'),
    '337AC0000000066': ('/tmp/ntsuusoku.xml', '国税通則法'),
    '340CO0000000097': ('/tmp/ext_340CO0000000097.xml', '法人税法施行令'),
    '340M50000040012': ('/tmp/ext_340M50000040012.xml', '法人税法施行規則'),
    '340CO0000000096': ('/tmp/ext_340CO0000000096.xml', '所得税法施行令'),
}
